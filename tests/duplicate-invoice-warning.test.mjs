...
const sql = await readFile(
  new URL("../supabase/subempreitadas_controle_contratual.sql", import.meta.url),
  "utf8",
);
...
assert.match(app.slice(app.indexOf('supabase("rpc\/fn_verificar_fatura_semelhante"'), app.indexOf("if \(!response.ok\)", app.indexOf('supabase("rpc\/fn_verificar_fatura_semelhante"'))), /p_obra_id: workId/);
...
assert.match(sql, /f\.fornecedor_id=p_fornecedor_id/);
assert.match(sql, /abs\(f\.valor-p_valor\)<=v_tolerancia/);
assert.match(sql, /outra_obra boolean/);
assert.match(sql, /f\.obra_id is distinct from p_obra_id/);
