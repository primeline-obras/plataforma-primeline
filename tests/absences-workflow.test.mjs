...
const interfaceMigration = fs.readFileSync(new URL("../supabase/aba_ausencias_interface_bloqueios.sql", import.meta.url), "utf8");

assert.match(app, /data-team-tab="vacations">MAPA DE FÉRIAS/);
assert.match(app, /data-team-tab="absences">AUSÊNCIAS/);
...
assert.match(app, /data-update-absence/);
assert.match(app, /function canManageAbsences/);
...
assert.match(interfaceMigration, /fn_pode_gerir_ausencias/);
assert.match(interfaceMigration, /trg_bloquear_horas_extra_ausencia/);
assert.match(interfaceMigration, /ausencias_anexos_equipa_insert/);
